							...
							triple indices)
{
	...
	indices->push_back(indices.v1);
	indices->push_back(indices.v2);
	indices->push_back(indices.v3);
	 
	// add the data of NUMBER_OF_VERTICES_PER_TRIANGLE vertices
	vertexData = new vector<GLfloat>();
	for (int i = 0; i < vertices->size(); i++)
	{
		vertexData->push_back(vertices->at(i)->getVertexData())
	}
}
